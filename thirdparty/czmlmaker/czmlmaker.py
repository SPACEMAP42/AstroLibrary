...
def color_coding_by_name_and_find_set(
    czml, target_name, outline_color=[255, 255, 255, 255], outline_width=1, pixel_size=4
):
    target_set = set()
...
def read_ppdb_for_RFI(ppdb_path):
    ...
    while input_line := src_file.readline():
        ...
        if (pid, sid) in pair_dict:
            pair_dict[(pid, sid)].append([start_time, tca_time, end_time, interfering])
        else:
...
if __name__ == "__main__":
    ...
    minute = int(minute)
    second = int(second)
    start_time = datetime(year, month, day, hour, minute, second, tzinfo=timezone.utc)
    late_start_time = start_time + timedelta(minutes=5)
    end_time = start_time + timedelta(hours=1)

    data_dir = os.getcwd()

    data_dir = data_dir + "/NURI-3"

    INPUT_TLE_PATH = f"{data_dir}/tle_all.txt"
    OUTPUT_CZML_PATH = f"{data_dir}/sat.czml"
    PRETTIER_CZML_PATH = f"{data_dir}/sat-prettier.czml"
    # PPDB_PATH = f"{data_dir}/PPDB-0526.txt"

    tle2czml.create_czml(
        inputfile_path=INPUT_TLE_PATH,
        start_time=start_time,
        end_time=end_time,
        outputfile_path=OUTPUT_CZML_PATH,
    )

    czml = czml_prettier(OUTPUT_CZML_PATH)

    # pair_dict, secondary_set = read_ppdb_for_RFI(PPDB_PATH)
    # pair_dict, primary_set, secondary_set = read_ppdb(PPDB_PATH)

    # czml = color_coding_by_id(
    #     czml, secondary_set, outline_color=[0, 255, 0, 255], outline_width=2, pixel_size=2
    # )

    czml, STARLINK_NORAD_ID = color_coding_by_name_and_find_set(
        czml, "STARLINK", outline_color=[0, 0, 255, 255]
    )

    # czml = color_coding_by_id(
    #     czml, target_set, outline_color=[0, 255, 0, 255], outline_width=4, pixel_size=5
    # )

    # czml = color_coding_by_id(
    #     czml, primary_set, outline_color=[0, 255, 0, 255], outline_width=4, pixel_size=5
    # )

    # czml = add_pair(czml, pair_dict)

    # czml = remove_disinterested_set(czml, primary_set, secondary_set, target_set)

    # target_id = "39227"
    # target_set = [target_id]

    # czml = color_coding_by_id(
    #     czml, target_set, outline_color=[0, 0, 255, 255], outline_width=4, pixel_size=5
    # )
    # czml = add_pair_for_RFI(czml, target_id, pair_dict)
    ...
    #     rgba=[240, 24, 24, 50],
    # )
    # pair_dict2, secondary_set2 = read_ppdb(PPDB_PATH2)
    # czml = color_coding_by_id(
    #     czml,
    #     secondary_set2,
    ...
    # czml = add_pair(czml, pair_dict)
    # czml = add_pair(czml, pair_dict2)

    # czml = remove_disinterested_set(czml, counter_set, counter_set2, swarm_norad_set, hawk_norad_set)

    # for hawk_id in hawk_norad_set:
    #     czml = add_cone(
    #         czml,
    #         f"{hawk_id}",
    #         start_time,
    ...
    #         [255, 255, 0, 85],
    #     )
    # for swarm_id in swarm_norad_set:
    #     czml = add_cone(
    #         czml,
    #         f"{swarm_id}",
    #         start_time,
    ...
    write_czml(PRETTIER_CZML_PATH, czml)
